...
namespace Lagrangian2dPara
{
    ...
    extern float supportRadius;// 在此半径内的相邻粒子会对当前粒子产生影响
    extern float particleRadius;// 粒子半径
    extern float particleDiameter;// 粒子直径
    extern float gravityX;// x轴上的加速度
    extern float gravityY;// y轴上的加速度
    extern float density;// 流体密度
    extern float stiffness;// 刚度
    extern float exponent;// 压力计算公式中的指数
...
namespace Lagrangian2dFountainPara
{
    ...
    extern glm::vec2 containerLower;   // 喷泉容器左下角（决定整体边界）
    extern glm::vec2 containerUpper;   // 喷泉容器右上角
    
    extern glm::vec2 emitterLower;     // 喷口区域左下角（贴近底部实现立柱）
    extern glm::vec2 emitterUpper;     // 喷口区域右上角（控制喷口宽度/高度）
    extern glm::vec2 emitterVelocity;  // 粒子初始速度（通常指向 +Y）
    extern float emitterJitter;        // 速度抖动强度（降低过度规则导致的条纹）
    
    extern float particleSpacing;      // 喷嘴内部粒子采样间距
    extern int particlesPerStep;       // 每帧发射的粒子数（影响水柱密度）
    extern size_t maxParticles;        // 粒子系统允许存在的最大数量
}
...
namespace Lagrangian3dPara
{
    ...
    extern float maxVelocity;// 最大允许速度
    extern float velocityAttenuation;// 碰撞后的速度衰减系数
    extern float eps;// 一个很小的距离，用于边界处理

    extern float supportRadius;// 支持半径
    extern float particleRadius;// 粒子半径
    extern float particleDiameter;// 粒子直径

    extern float gravityX;// x轴重力
    extern float gravityY;// y轴重力
    extern float gravityZ;// z轴重力
